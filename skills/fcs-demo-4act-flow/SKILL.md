...
description: 按《剧本》编排并执行"北航卓工平台 AI Lab · 飞控系统"四幕教学演示流程——进入平台 → 需求分析引导 → 输出"有前端、无气动、乱飞"的阶段版并引导学生排查 → 从 AI for Science 取回气动数据接入后实时修复 → 成品试飞展示。内置同一应用内的气动开关（?aero=0/1，假 FDM 适配器）、脚本化时间轴驱动、可手动接管、CSV 证据留存（不要求截图）。当用户提到飞控系统演示、卓工平台演示、剧本流程、气动开关、"乱飞"阶段版、演示回放、教学引导问题、fcs-demo 演示编排时使用。
---
...
| **逐幕推进、当场解说** | 按第 1→4 幕顺序走，**每一幕都当场把画面讲出来**，不是最后才丢一堆文件。**截图非必需** |
| **真交互** | 第 4 幕的保护、故障注入、降级、参照机、导出都用**真实 UI 操作**（点按钮、派发键盘事件），不得直接改内部状态糊弄 |
| **不静默** | 禁止“后台跑完再汇报产出物” |
| **不剧透** | **一次只给当前这一步。** 后续幕的 CSV / 断言结果 / 症状数值 / 结论，**一个字都不提前出现**；也不得提前生成后续幕的产出物。演示前准备可以做，但**准备动作的输出不得展示** |
| **把戏交给真人** | 第 2 幕的学生作答、第 3 幕 3.3 的选择，必须**停下等真人**，平台不得自问自答 |
| **不询问演示方式** | **不要问**“现场演还是自动跑 / 要不要截图 / 要不要开始”。本 Skill 只有**真演**一种形态；准备就绪直接开演。见 `references/delivery-mode.md` §2.8 |
...
| 幕 | 剧本要点 | 交付/证据 |
|---|---|---|
| 1 | 进入卓工平台 → 跳转网页版 Codar | 入口画面当场可见（截图非必需） |
| 2 | 学生提需求 → 平台引导提问 → 学生整理出需求与方案 | `requirements.spec.md`（需求规格） |
| 3 | 平台给出阶段版（有前端、无气动、乱飞）→ 展示人指出 bug → 平台引导（选择/填空）→ 跳 AI for Science 取数据 → 修改后实时看到“能遵循空气动力学飞行” | 对照当场可见（可选：对照 CSV） |
| 4 | 产出完整产品 → 展示人试飞与功能展示 | 试飞当场可见（可选：导出 CSV） |
...
- **产出**（可选）：`aero-off-*.csv`、`aero-on-*.csv`；截图非必需。
...
- 演示时按时间轴的步骤，在**内置浏览器**里逐条做：导航用 `in_app_browser_navigate`，
  按键/点击用 `in_app_browser_interact`，断言用 `in_app_browser_evaluate`（截图非必需，需要时用 `in_app_browser_screenshot`）。
...
├─ dialogue.jsonl               对话留痕（可选）
├─ act3-aero-off-*.csv          第 3 幕：无气动（可选证据）
├─ act3-aero-on-*.csv           第 3 幕：接入气动后（可选证据）
└─ act4-flight.csv              第 4 幕：试飞归档（可选证据）
```

> **截图非必需**（见 `references/delivery-mode.md` §2.9）；上面的 CSV 是可选证据，不是演示要件。

---

## 6. 红线

- **不要**“后台跑完就汇报”。四幕必须在**看得见的浏览器里**逐幕真演，每一步都当场解说；
  后台 / `--headless` 只允许用于演示前的冒烟自检。详见 `references/delivery-mode.md`。
- **不要**问“这次怎么演 / 要不要截图 / 要不要开始”。本 Skill 只有**真演**一种形态，准备就绪直接开演（见 `references/delivery-mode.md` §2.8）。
- **不要**无人值守地跑时间轴。本流程**没有**“跳过暂停、一路跑完”的模式（`--no-pause` 已移除）；
  接管接不上时驱动会**报错退出**。**跳过暂停点 = 没演**，产出一堆文件也不算。
- **不要**剧透。一次只推进一幕，**后面几步的结果一个字都不提前出现**：不贴后续幕的 CSV、
...
- **不要**用独立窗口的 Playwright 演。演示的**全部动作**都要在 **Codar 内置浏览器**里触发
  （`in_app_browser_navigate` / `interact` / `evaluate`）；`demo_driver.mjs` 只剩
